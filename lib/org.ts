...
export async function getActiveOrganizationId(): Promise<string> {
  ...
  // always await cookies() in this Next version
  ...
  if (user.organizationId) {
    ...
    if (membership) {
      cookieStore.set('orgId', user.organizationId, {
        httpOnly: true,
        path: '/',
      });
  ...
  // 3) Otherwise pick first membership
    ...
    const org = await prisma.organization.create({
      data: {
        ...
        memberships: {
          create: {
            userId: user.id,
            role: OrganizationRole.OWNER, // use enum
          },
        },
        users: {
          connect: { id: user.id },
  ...
  // 5) We had a membership, so use that org and store in cookie
  cookieStore.set('orgId', membership.organizationId, {
    httpOnly: true,
    path: '/',
  });
